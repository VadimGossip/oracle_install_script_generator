...
def check_rename(entity_name):
    if entity_name.find('=>') != -1:
        return entity_name.split('=>')[1].replace('}', '').replace(' ','')
    else:
        return entity_name   
...
def scan_git_for_changed_objects():
    ...
    for commit in filtered_commit_list:
        ...
        for item in commit.stats.files:
            ...
            epic_module_name = check_rename(path_parser.extract_info_from_path_list(path_list, 'epic_module_name'))
            module_name = check_rename(path_parser.extract_info_from_path_list(path_list, 'module_name'))
            object_type = check_rename(path_parser.extract_info_from_path_list(path_list, 'object_type'))
